...
\begin{itemize}
    \item added \Rfunction{inactiveTargets} which finds targets a compound is inactive against
    \item added \Rfunction{targetSelectivity} which computes target selectivity for a list of compounds
    \item added \Rfunction{screenedAtLeast} which returns a list of CIDs screened at least X times
    \item added \Rfunction{loadIdMapping} which stores protein identifier mappings
    \item added \Rfunction{translateTargetId} which retrieves stored protein identifier mappings
\end{itemize}
...
First, create a new database. For purposes of this manual a temporary file is used, but you can replace the \Rfunction{tempfile} function call with a filename of your choice if you wish to save the resulting database for later.
...
This \Rclass{data.frame} must contain four columns which includes a cid (unique compound identifier) for each compound, an sid (often used to distinguish distinct samples of the same compound structure), a binary activity score ($1=active$, $0=inactive$), and a numeric activity score.
...
\noindent After indexing, perform a test query on your database to confirm that the data loaded correctly.
...
\section{Cluster Compounds by Activity Profile}
...
In many cases it is too cpu and memory intensive to cluster all compounds in the database, so we first pull just a subset of these data from the database into an \Robject{bioassaySet} object, and then convert that into a compounds vs targets activity matrix for subsequent clustering according to similarities in activity profiles.
The function \Rfunction{getBioassaySetByCids} extracts the activity data for a given list of compounds. Alternatively, the entire data for a given list of assay ids can be extracted with the function \Rfunction{getAssays}.
...
\noindent
Next, select data from just 3 compounds to extract into a \Robject{bioassaySet} object for subsequent analysis.
...
\noindent
The function \Rfunction{perTargetMatrix} converts the activity data extracted earlier into a matrix of targets (rows) vs compounds (columns). Data from multiple assays hitting the same target is summarized by giving a "1" (active) if any of these data are active. Both inactive scores and untested combinations are assigned zeros (inactive) to create a ``complete matrix.'' We caution the user to carefully consider if this step makes sense within the context of the specific experiments being analyzed. Here a sparse matrix (which omits actually storing 0 values) is used to save memory.
...
\section{Analyze and Load Raw Screening Data}
...
\noindent
This example is based on the \Rpackage{cellHTS2} library. Example data
is used which is included with \Rpackage{cellHTS2}. This is actually data from
...
\noindent
Convert the binary calls into an activity table that \Rpackage{bioassayR} can parse.
...
\noindent
Create a new (temporary in this case) \Rpackage{bioassayR} database to load these data into.
...
\noindent
Load this assay object into the \Rpackage{bioassayR} database.
