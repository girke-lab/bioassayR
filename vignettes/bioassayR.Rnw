...
\begin{itemize}
    \item added \Robject{bioassaySet} object for storing multiple assays in memory
    \item added workflow example for raw screening data
    \item added \Robject{perTargetMatrix} function to compute a sparse matrix summarizing select bioactivity data
\end{itemize}
