...
The function *perTargetMatrix* converts the activity data extracted earlier into a matrix of targets (rows) vs compounds (columns). Data from multiple assays hitting the same target are summarized into a single value in the manner specified by the user. If you choose the *conflictResolver* option
...
```{r eval=TRUE, tidy=FALSE}
myActivityMatrix <- perTargetMatrix(selectedAssayData, inactives=TRUE, conflictResolver = "mode")
...
We also reccomend the book ``Using SQLite'' by Jay A. Kreibich.
